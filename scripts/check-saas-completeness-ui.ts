...
function main() {
  ...
  assert(reviewResult.includes("buildExcelReportBlob") && reviewResult.includes('reportFilename("xlsx")'), "Excel 按钮导出标准 XLSX 文件")
  assert(!reviewResult.includes("buildReportExcelHtml") && !reviewResult.includes('reportFilename("xls")'), "Excel 导出不再使用 HTML 伪 XLS 格式")
  assert(reviewResult.includes("buildWordReportBlob") && reviewResult.includes('reportFilename("docx")'), "Word 按钮导出标准 DOCX 文件")
  assert(!reviewResult.includes("buildWordReportHtml"), "Word 导出不再使用 HTML 伪 DOC 格式")
