...
import { PROFESSION_TYPES, type ProfessionType } from "./professions/profession-config"

export { PROFESSION_TYPES, type ProfessionType } from "./professions/profession-config"
...
export const CHUNK_CONFIG = {
  MAX_CHARS_PER_CHUNK: 250000,          // DeepSeek 1M 上下文下的智能分块目标
  MAX_TOTAL_CHARS_FOR_NORMAL: 800000,   // 超大方案才触发分块审核
  MAX_KNOWLEDGE_PER_CHUNK: 80000,       // 每块锚点/知识上下文预算
...
export function shouldUseChunkedReview(
  ...
  // 按文件大小兜底判断：文本解析后仍很短时不再仅因 10MB 扫描 PDF 触发分块；
  // 真正的超大扫描件通常文件与解析文本都很大，这里只保留极端保护。
  if (fileSize && fileSize > 80 * 1024 * 1024) {
    return {
      needsChunking: true,
      reason: `文件大小 ${(fileSize / 1024 / 1024).toFixed(1)}MB 超过 80MB 兜底阈值`
    }
  }
