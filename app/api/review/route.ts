...
（基于全文审核结果填写，须在各章节审核完成后总结，与各章问题一致，不得与各章矛盾）
...
4. **意见独立性**（严禁重复充数）：每条审核意见必须独立、问题各不相同，**不得输出问题描述/依据/整改完全相同或高度相似的两条意见**。若某主题只有一种问题，就只出一条；宁少勿滥，不为凑数重复。

5. **审核判断逻辑**（避免对已满足要求的方案挑刺）：方案参数**严于**规范值（如规范要求夹角>45°、方案取70°）→ 输出"满足要求"，可建议补充设计理由，**不得表述为"表述不严谨/不规范"**；方案参数**等于**规范值 → 输出"满足要求"；方案参数**低于/违反**规范值 → 输出"不符合，须整改"并给整改建议。
...
**主要问题概览：** （列出最关键的 2-3 条，如"①未提供基坑稳定性验算书；②监测报警值设置错误"）
...
3. 整改建议必须具体可执行，不得使用"建议完善"、"建议补充"等模糊表述。
4. **审核判断逻辑**（避免对已满足要求的方案挑刺）：方案参数**严于**规范值（如规范要求夹角>45°、方案取70°）→ 输出"满足要求"，可建议补充设计理由，**不得表述为"表述不严谨/不规范"**；方案参数**等于**规范值 → 输出"满足要求"；方案参数**低于/违反**规范值 → 输出"不符合，须整改"并给整改建议。
5. 语言风格：专业、客观、准确。
6. 【禁止事项】以下行为严格禁止：
   - 禁止编造锚点库中不存在的规范文件名称或编号
   - 禁止编造规范中不存在的条款号
